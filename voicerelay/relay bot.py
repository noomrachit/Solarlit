...
def _patch_voice_recv_resilience():
    ...
    # prefsize=1 บางมาก แปลว่าแพ็กเก็ตที่มาช้า/สลับลําดับแม้แค่นิดเดียว (jitter ปกติของเน็ตจริง
    # โดยเฉพาะ path ไกลๆ) ถูกนับว่า "หลุด" ทันที เกิด log "packets were lost being flushed" ถี่ๆ
    # ระหว่างที่คนกําลังพูดอยู่ต่อเนื่อง (คนละจุดกับตอนเริ่มพูดใหม่ที่ warm-up mute ข้างบนดูแลอยู่แล้ว)
