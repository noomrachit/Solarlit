...
async def _post_profile_embed(interaction: discord.Interaction):
    ...
    settings = await get_settings(interaction.guild.id)
    channel_id = settings.get("intro_channel")
...
async def setup_introduction(interaction: discord.Interaction, channel: discord.TextChannel):
    pool = await db.get_pool()
    await pool.execute("""
        INSERT INTO settings (guild_id, intro_channel) VALUES ($1, $2)
...
BOT_DASHBOARD = [
    {
        ...
        "commands": [
            ...
            "/setup-introduction, /setup-jobs, /my-profile, /edit-profile, /player-search",
        ],
    },
    {
        "name_match": "ยาม",
