...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    """Handle all slash command errors automatically."""
    ...
    # Reply safely (handle already-responded cases)
    try:
...
async def on_error(event: str, *args, **kwargs):
    """Catch unhandled errors in events."""
    log.exception(f"Unhandled error in event `{event}`")

@bot.event
async def on_command_error(ctx: commands.Context, error: commands.CommandError):
    """Fallback for prefix commands (if any)."""
...
@welcome_group.command(name="message", description="ตั้งข้อความต้อนรับ (รองรับ {mention} {user} {name} {server} {count})")
@has_mod_perms()
